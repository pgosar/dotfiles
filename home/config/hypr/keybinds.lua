...
hl.bind(
  mainMod .. " + SHIFT + S",
  hl.dsp.exec_cmd('HYPRSHOT_DIR="$HOME/Pictures/Screenshots/" hyprshot -m region')
)
...
local function switch_workspace(i)
  ...
  if active_monitor and active_monitor.description then
    ...
    if active_monitor.description == m2_desc then is_monitor2 = true end
  end

  local target_ws = i
  if is_monitor2 then target_ws = i + 10 end
  ...
  if i == 10 then
    ...
    if active_monitor_name then hl.dispatch(hl.dsp.focus({ monitor = active_monitor_name })) end
    return
  end
